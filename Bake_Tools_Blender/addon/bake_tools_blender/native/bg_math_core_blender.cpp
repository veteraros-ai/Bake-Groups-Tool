...
// Blender passes evaluated world-space vertex snapshots.  Reject malformed or
// non-finite buffers at the Python boundary: the original Maya build trusted
// its MFnMesh extractor, while Blender modifiers and Geometry Nodes can expose
// invalid coordinates that must not reach the spatial-grid index.
static void validate_point_cloud(const std::vector<float>& values, const char* name) {
    if ((values.size() % 3) != 0) {
        throw std::invalid_argument(std::string(name) + " must contain xyz triplets");
    }
...
static void validate_triangles(const std::vector<float>& values, const char* name) {
    if ((values.size() % 9) != 0) {
        throw std::invalid_argument(std::string(name) + " must contain flattened triangle xyz data");
    }
    validate_point_cloud(values, name);
}
...
float calculate_avg_distance(const std::vector<float>& lp_verts, const std::vector<float>& hp_verts) {
    ...
    std::unique_ptr<SpatialGrid> grid = use_grid
        ? std::make_unique<SpatialGrid>(hp_verts)
        : nullptr;
...
py::dict py_calculate_surface_match(
    ...
    float tolerance)
{
    validate_point_cloud(hp_samples, "hp_samples");
    validate_triangles(hp_triangles, "hp_triangles");
    validate_point_cloud(lp_samples, "lp_samples");
    validate_triangles(lp_triangles, "lp_triangles");
    if (!(tolerance > 0.0f) || !std::isfinite(tolerance)) {
        throw std::invalid_argument("tolerance must be a positive finite number");
    }
...
float py_calculate_avg_distance(const std::vector<float>& lp_verts, const std::vector<float>& hp_verts) {
    validate_point_cloud(lp_verts, "lp_verts");
    validate_point_cloud(hp_verts, "hp_verts");
    py::gil_scoped_release release;
...
float py_calculate_bidirectional_avg_distance(
    ...
    const std::vector<float>& verts_b)
{
    validate_point_cloud(verts_a, "verts_a");
    validate_point_cloud(verts_b, "verts_b");
    py::gil_scoped_release release;
...
bool check_mesh_collision(const std::vector<float>& verts_a, const std::vector<float>& verts_b, float threshold) {
    ...
    if (!(threshold > 0.0f) || !std::isfinite(threshold)) return false;

    // Maya's old build returned true when two occupied hash cells merely
    // touched, which produced false positives up to several thresholds away.
    // Blender uses the grid only as a candidate index and confirms the actual
    // Euclidean vertex distance.
    float cell_size = threshold;
            ...
            for (int dy = -1; dy <= 1; ++dy) {
                for (int dz = -1; dz <= 1; ++dz) {
                    ...
                    for (size_t index : it->second) {
                        float ddx = verts_b[i] - verts_a[index];
                        float ddy = verts_b[i + 1] - verts_a[index + 1];
                        float ddz = verts_b[i + 2] - verts_a[index + 2];
...
bool py_check_mesh_collision(const std::vector<float>& verts_a, const std::vector<float>& verts_b, float threshold) {
    ...
    if (!(threshold > 0.0f) || !std::isfinite(threshold)) {
        throw std::invalid_argument("threshold must be a positive finite number");
    }
    py::gil_scoped_release release;
...
int py_resolve_hp_collision(const std::vector<float>& hp_verts, const std::vector<std::vector<float>>& lp_candidates_verts) {
    validate_point_cloud(hp_verts, "hp_verts");
    for (const auto& values : lp_candidates_verts) {
        validate_point_cloud(values, "lp_candidate");
    }
    py::gil_scoped_release release;
...
std::vector<std::tuple<int, int, float, float, int, float>> py_calculate_vertex_owner_scores(
    ...
    const std::vector<std::pair<int, int>>& candidate_pairs)
{
    for (const auto& values : lp_point_sets) validate_point_cloud(values, "lp_point_set");
    for (const auto& values : hp_point_sets) validate_point_cloud(values, "hp_point_set");
    py::gil_scoped_release release;
...
MeshMetrics py_analyze_mesh_shape(const std::vector<float>& verts) {
    validate_point_cloud(verts, "verts");
    py::gil_scoped_release release;
    return analyze_mesh_shape(verts);
}

std::string py_generate_fingerprint_data(
    const std::vector<float>& verts,
    const std::vector<float>& center)
{
    validate_point_cloud(verts, "verts");
    if (center.size() < 3) throw std::invalid_argument("center must contain xyz");
    for (float value : center) {
        if (!std::isfinite(value)) throw std::invalid_argument("center contains NaN or infinity");
    }
    py::gil_scoped_release release;
...
PYBIND11_MODULE(BG_MATH_CORE_MODULE_NAME, m) {
    m.doc() = "Blender CPython native math accelerator for Bake Tools";
